...
public class FlowCalibrationController implements Initializable {


	double conditionflow1,conditionflow2, conditionpressure;
	...
	List<Double> pressuredata1,flowdata1,pressuredata2,flowdata2,pressuredata3,flowdata3;
	...
	int type=0;  // 0 = flow1 , 1 = flow2 , 2=mix
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		conditionflow1 = (double) Double.parseDouble(DataStore.getFm1()) * 0.80;
		conditionflow2 = (double) Double.parseDouble(DataStore.getFm2()) * 0.80;
		

		
		pressuredata1=new ArrayList<Double>();
		flowdata1=new ArrayList<Double>();
		pressuredata2=new ArrayList<Double>();
		flowdata2=new ArrayList<Double>();
		pressuredata3=new ArrayList<Double>();
		flowdata3=new ArrayList<Double>();
		...
		sc.setTitle("Pressure Regulator Vs Flow Meter");
		sc.prefWidthProperty().bind(root.widthProperty());
		sc.prefHeightProperty().bind(root.heightProperty());

		xAxis.setLabel("Flow Meter ( psi )");
		yAxis.setLabel("Pressure Regulator ( psi ) ");
...
void setFlowCalibrationPoint(double p,double f) 
	{	
		...
		if(ind>0)
		{
					Platform.runLater(new Runnable() {
						...
						public void run() {
							...
							lbltime.setText(Myapp.getRound(f, 2)+" Sccm");
							...
							if (p >= conditionpressure || f>=conditionflow1) {
								try {
		...
		if(ind>0)
		{
					Platform.runLater(new Runnable() {
						...
						public void run() {
							...
							flowdata2.add(f);
							ind++;
							if (p >= conditionpressure || f>=conditionflow2) {
								try {
	...
	void createCsvTable() {
		...
		try {
			...
			File f = new File(fff.getPath() + "/"+"Flow");
			if (!f.isDirectory()) {
				f.mkdir();
			...
			cs.wtirefile(f.getPath()+ "Flow_" + findInt(ff)
					+ ".csv");

			System.out.println("File Path-->" + f.getPath());

			cs.firstLine("flowcalibration");
			...
			cs.newLineDouble("flow1", flowdata1);
			cs.newLineDouble("pressure1", pressuredata1);

			cs.newLineDouble("flow2", flowdata2);
			cs.newLineDouble("pressure2", pressuredata2);

			cs.newLineDouble("flow", flowdata3);
			cs.newLineDouble("pressure", pressuredata3);
