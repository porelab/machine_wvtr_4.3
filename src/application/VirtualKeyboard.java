...
//    final Button tab = createNonshiftableButton("Tab", KeyCode.TAB, modifiers, target);
    ...
    enter.setOnAction(new EventHandler<ActionEvent>() {
		...
		public void handle(ActionEvent event) {
			...
			if(Myapp.nextTf!=null)
			{
				Myapp.lblkeyboard.setText(Myapp.nextlbl);
				Myapp.currTf
  ...
  //  cursorUpDown.getChildren().addAll(cursorUp, cursorDown);
    ...
    final Node[][] extraLeftButtons = new Node[][] {  {modifiers.capsLockKey()} };
    final Node[][] extraRightButtons = new Node[][] { {backspace}, {enter} };
  ...
  private Button createShiftableButton(final String unshifted, final String shifted,
    ...
    final StringBinding text = 
        Bindings.when(modifiers.capsLockOn().and(letter))
        .then(shifted)
        .otherwise(unshifted);
